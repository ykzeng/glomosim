                      ...
                      float rxAntennaHeight)
{
    ...
    if (valueForPlaneEarthLoss > valueForFreeSpaceLoss) {
        if (valueForPlaneEarthLoss > 1.0) {
            pathloss += 10.0 * alpha * log10(valueForPlaneEarthLoss);
        }
    } else {
        if (valueForFreeSpaceLoss > 1.0) {
            pathloss += 10.0 * alpha * log10(valueForFreeSpaceLoss);
        }
    }
    // add gaussian noise to two ray model to make it log-normal shadowing
    pathloss += box_muller(0.0, 0.0);
